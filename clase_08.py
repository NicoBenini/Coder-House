...
#Ejemplo de la diapositiva programa de mascota

def guardar_contacto_en_agenda(nombre, raza, agenda):
    agenda[nombre.upper()] = raza.capitalize() 
    return agenda
...
NuevoDato = str(input("¿Desea introducir un nuevo dato (S o N): "))
while NuevoDato == "S":
    
    nombre = input("¿Cual es el nombre de tu mascota? ")
    raza = input("¿Cual es la raza de tu mascota? ")
    mi_agenda = guardar_contacto_en_agenda(nombre, raza, mi_agenda)
    NuevoDato = str(input("¿Desea introducir un nuevo dato (S o N): "))
   
else:
    mi_archivo = open("miMAscota.txt", "w")
    mi_archivo.write(str(mi_agenda))
    mi_archivo.close

print("programa corrió bien!")
